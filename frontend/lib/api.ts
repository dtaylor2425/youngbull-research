...
export async function getPremiumWorkbook(
  ticker: string
): Promise<PremiumWorkbook | null> {
  try {
    return await request<PremiumWorkbook>(
      `/api/stocks/${encodeURIComponent(ticker.toUpperCase())}/premium`
    );
  } catch {
    return null;
  }
...
export function getComparison(ticker: string) {
  return request<ComparisonResponse>(
    `/api/stocks/${encodeURIComponent(ticker.toUpperCase())}/comparison`
  );
}
